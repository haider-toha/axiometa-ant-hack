# =============================================================================
# BUS-STOP SITUATIONAL-AWARENESS ENCLOSURE — Fusion 360 Python Script
# =============================================================================
...
#   3. Select "bus_stop_enclosure" -> "Run"
...
#   "BusStopEnclosure".  The script produces TWO bodies (cage + skin_plate) --
...
#   T1:audit/01:<what>       measured in audit/bus-stop-situational-awareness/
#                            01-track-1-physical-cad-ground-truth.md
#   P1:audit/07:§<n>         measured/derived in audit/bus-stop-situational-
...
GRILLE_RING_N    = 6       # ring hole count; +1 centre hole = 7   P1:audit/07:§5.3
                           ...
                           #   arrives at ~50-60 dB, conversation level, at a bus
                           #   stop.  The one designer who has actually printed a
...
def _cut_button_slot():
    ...
    (plan/2026-07-18-bus-stop-situational-awareness.md).  Sealing them would
    ...
    detail -- see audit/bus-stop-situational-awareness/08 §Residual risk."""
    sk = _sketch_on_xy_at(Z_THRU_BOT)
...
def run(context):
    ...
    try:
        ...
        # --- Report -----------------------------------------------------------
        msg = ("Bus-stop situational-awareness enclosure built (CLOSED MONOLITH).\n\n"
...
#
#     .venv/bin/python cad/bus_stop_enclosure.py
#
...
def _headless_main():
    ...
    asm.label = "BusStopEnclosure"
    here = os.path.dirname(os.path.abspath(__file__))
    step_path = os.path.join(here, "bus_stop_enclosure.step")
    stl_path = os.path.join(here, "bus_stop_enclosure.stl")
